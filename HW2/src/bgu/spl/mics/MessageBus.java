...
/**
 * @inv: foreach micro-service m: size(m)>=0
 */
     ...
     * @pre: isCompleted(Event<T> e, T result)==false
     * @post: isCompleted(Event<T> e, T result)==true
     */
     ...
     * @pre: foreach micro-service m: size(m)>=0
     * @post: foreach micro-service m: size(m)==@pre(size(m)+1)
     */
     ...
     * @pre : foreach micro-service m: size(m)>=0
     * @post: foreach micro-service m{if(isSubscribedEvent(Class<? extends Event<T>> type,m)) size(m)==@pre(size(m)+1) }
     */
     ...
     * @pre: isResgistered(m)==false
     * @post: isResgistered(m)==true
     * @post: size(m)==0
     */
     ...
     * @pre: isResgistered(m)==true
     * @post: isResgistered(m)==false
     */
    ...
    /**
     * retrieves the size of the queue allocated to {@code m} via the call to
     * {@link #register(bgu.spl.mics.MicroService)}
     * @param m the requested micro-service
     * @return the size of the requested micro-service's queue
     */
    int size(MicroService m);

    /**
     *
     * @param type the type of event subscribed
     * @param m the micro-service to check if subscribed
     * @param <T> The type of the result expected by the completed event.
     * @return true if m subscribed, false otherwise
     */
    <T> boolean isSubscribedEvent(Class<? extends Event<T>> type, MicroService m);

    /**
     *
     * @param type the type of broadcast subscribed
     * @param m the micro-service to check if subscribed
     * @return true if m subscribed, false otherwise
     */
    ...
    /**
     *
     * @param m requested micro service
     * @return true if m is registered, false otherwise
     */
     ...
     * @return true if the relevant future object is resolved,false otherwise.
     */
    <T> boolean isCompleted(Event<T> e, T result);

    /**
     * retrieving the next message in m queue
     * @param m the requesting micro-service
     * @return returns the next message in m queue
     */
    Message getNextMessage(MicroService m);
